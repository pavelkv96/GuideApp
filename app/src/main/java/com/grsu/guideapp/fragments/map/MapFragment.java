...
import com.grsu.guideapp.databases.DatabaseHelper;
...
public class MapFragment extends BaseMapFragment<MapPresenter, NavigationDrawerActivity>
        implements OnMapReadyCallback, TileProvider, MapViews {

    private final String TAG = getTags();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        if (getArguments() != null) {
            route = getArguments().getInt("KEY_ID_ROUTE");
        }
        ...
        if (route != -1) {
            ...
            mPresenter.setType(new long[]{1, 4});
            //openDialogViews();
        } else {
            if (getActivity != null) {
    ...
    public void setStartMarker(LatLng startMarker) {
        MarkerOptions markerOptions = new MarkerOptions().position(startMarker);
        mMap.addMarker(markerOptions);
    }

    @Override
    public void setPointsTurn(LatLng pointsTurn) {
    ...
    public void setEndMarker(LatLng endMarker) {
        MarkerOptions markerOptions = new MarkerOptions().position(endMarker);
        mMap.addMarker(markerOptions);
    }

    @Override
    public void setPolyline(List<LatLng> pointsList, int id) {
    ...
    @Override
    public void setCurrentPoint(LatLng currentPoint) {
        if (current == null) {
            MarkerOptions markerOptions = new MarkerOptions().position(currentPoint);
            current = mMap.addMarker(markerOptions);
        }
        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(currentPoint, 18));
        current.setPosition(currentPoint);
    }

    @Override
    public void setPoi(Poi poi) {
        int icon = /*MapUtils.getIconByType(poi.getType())*/R.mipmap.ic_launcher;
        MarkerOptions markerOptions = new MarkerOptions()
