...
public class BottomSheetBehaviorGoogleMaps<V extends View> extends Behavior<V> {
     ...
     */
    public BottomSheetBehaviorGoogleMaps() {
    }
     ...
     */
    public BottomSheetBehaviorGoogleMaps(Context context, AttributeSet attrs) {
        super(context, attrs);
        int[] behavior_layout = android.support.design.R.styleable.BottomSheetBehavior_Layout;
        int layout_behavior_peekHeight = android.support.design.R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight;
        int layout_behavior_hideable = android.support.design.R.styleable.BottomSheetBehavior_Layout_behavior_hideable;

        TypedArray a = context.obtainStyledAttributes(attrs, behavior_layout);
        setPeekHeight(a.getDimensionPixelSize(layout_behavior_peekHeight, 0));
        setHideable(a.getBoolean(layout_behavior_hideable, false));
        a.recycle();
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (mState == STATE_ANCHOR_POINT) {
                    ...
                    if (scroll != null && parent
                            .isPointInChildBounds(scroll, initialX, mInitialY)) {
                        mActivePointerId = event.getPointerId(event.getActionIndex());
                        mTouchingScrollingChild = true;
                    }
                }
        ...
        if (dy > 0) { // Upward
            ...
            if (!ViewCompat.canScrollVertically(target, -1)) {
                if (newTop <= mMaxOffset || mHideable) {
                    ...
                    if (mCollapsible == true || (mCollapsible == false
                            && (mAnchorPoint - newTop) >= 0)) {
                        consumed[1] = dy;
                        ViewCompat.offsetTopAndBottom(child, -dy);
                        setStateInternal(STATE_DRAGGING);
                    }
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target,
        ...
        if (scrollVelocity > mMinimumVelocity) {
            ...
            // Not flinging, just settle to the nearest state
                ...
                // Collapse?
                int currentTop = child.getTop();
                if (currentTop > mAnchorPoint * 1.25 && mCollapsible
                        == true) { // Multiply by 1.25 to account for parallax. The currentTop needs to be pulled down 50% of the anchor point before collapsing.
                    top = mMaxOffset;
                    targetState = STATE_COLLAPSED;
                }
                // Expand?
     ...
     * animation.
     *
     * @param state One of {@link #STATE_COLLAPSED}, {@link #STATE_ANCHOR_POINT}, {@link
     * #STATE_EXPANDED} or {@link #STATE_HIDDEN}.
     */
     ...
     *
     * @return One of {@link #STATE_EXPANDED}, {@link #STATE_ANCHOR_POINT}, {@link
     * #STATE_COLLAPSED}, {@link #STATE_DRAGGING}, and {@link #STATE_SETTLING}.
     */
    @State
...
//            mCallback.onStateChanged(bottomSheet, state);
    ...
    /**
     * A utility function to get the {@link BottomSheetBehaviorGoogleMaps} associated with the
     * {@code view}.
     *
     * @param view The {@link View} with {@link BottomSheetBehaviorGoogleMaps}.
     * @param <V> Instance of behavior
     * @return The {@link BottomSheetBehaviorGoogleMaps} associated with the {@code view}.
     */
    @SuppressWarnings("unchecked")
    public static <V extends View> BottomSheetBehaviorGoogleMaps<V> from(V view) {
        ...
        CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) params)
                .getBehavior();
        if (!(behavior instanceof BottomSheetBehaviorGoogleMaps)) {
            throw new IllegalArgumentException(
                    "The view is not associated with BottomSheetBehaviorGoogleMaps");
        }
        return (BottomSheetBehaviorGoogleMaps<V>) behavior;
    }
