...
public class Switch extends View {
    ...
    private int trackColorKey = Color.parseColor("#fff57e7e");
    private int trackCheckedColorKey = Color.parseColor("#ff52ade9");
    private int thumbColorKey = Color.parseColor("#ffffffff");
    private int thumbCheckedColorKey = Color.parseColor("#ffffffff");
    ...
    public void setDrawRipple(boolean value) {
        ...
        if (rippleDrawable == null) {
            ...
            if (Build.VERSION.SDK_INT >= 23) {
                ...
                maskDrawable = new Drawable() {
                    ...
                    public void draw(Canvas canvas) {
                        ...
                        canvas.drawCircle(bounds.centerX(), bounds.centerY(), dp(18), ripplePaint);
                    }
            ...
            if (Build.VERSION.SDK_INT >= 23) {
                rippleDrawable.setRadius(dp(18));
            }
            rippleDrawable.setCallback(this);
        }
        if (isChecked && colorSet != 2 || !isChecked && colorSet != 1) {
            int color = isChecked ? Color.parseColor("#21024781") : Color.parseColor("#17404a53");
        ...
        if (Build.VERSION.SDK_INT >= 28 && value) {
            rippleDrawable.setHotspot(isChecked ? 0 : dp(100), dp(18));
        }
    ...
    public void setIcon(int icon) {
        if (icon != 0) {
            ...
            if (iconDrawable != null) {
                iconDrawable.setColorFilter(new PorterDuffColorFilter(lastIconColor = isChecked ? trackCheckedColorKey : trackColorKey, PorterDuff.Mode.MULTIPLY));
            }
        } else {
            iconDrawable = null;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int tx = x + dp(7) + (int) (dp(17) * progress);
        ...
        for (int a = 0; a < 2; a++) {
            ...
            rectF.set(x, y, x + width, y + dpf2(14));
            canvasToDraw.drawRoundRect(rectF, dpf2(7), dpf2(7), paint);
            canvasToDraw.drawCircle(tx, ty, dpf2(10), paint);

            if (a == 0 && rippleDrawable != null) {
                rippleDrawable.setBounds(tx - dp(18), ty - dp(18), tx + dp(18), ty + dp(18));
        ...
        for (int a = 0; a < 2; a++) {
            ...
            canvasToDraw.drawCircle(tx, ty, dp(8), paint);
            ...
            if (a == 0) {
                if (iconDrawable != null) {
                    ...
                    tx -= dp(10.8f) - dp(1.3f) * progress;
                    ty -= dp(8.5f) - dp(0.5f) * progress;
                    int startX2 = (int) dpf2(4.6f) + tx;
                    int startY2 = (int) (dpf2(9.5f) + ty);
                    int endX2 = startX2 + dp(2);
                    int endY2 = startY2 + dp(2);

                    int startX = (int) dpf2(7.5f) + tx;
                    int startY = (int) dpf2(5.4f) + ty;
                    int endX = startX + dp(7);
                    int endY = startY + dp(7);
                    ...
                    startX = (int) dpf2(7.5f) + tx;
                    startY = (int) dpf2(12.5f) + ty;
                    endX = startX + dp(7);
                    endY = startY - dp(7);
                    canvasToDraw.drawLine(startX, startY, endX, endY, paint2);
                } else if (drawIconType == 2 || iconAnimator != null) {
                    paint2.setAlpha((int) (255 * (1.0f - iconProgress)));
                    canvasToDraw.drawLine(tx, ty, tx, ty - dp(5), paint2);
                    canvasToDraw.save();
                    canvasToDraw.rotate(-90 * iconProgress, tx, ty);
                    canvasToDraw.drawLine(tx, ty, tx + dp(4), ty, paint2);
